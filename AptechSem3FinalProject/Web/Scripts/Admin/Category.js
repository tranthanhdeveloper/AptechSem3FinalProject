...
{
    $('.tree li:has(ul)').addClass('parent_li_lecture').find(' > span').attr('title', 'Collapse this branch');
    $('.tree li.parent_li_lecture').each(function (e) {
        var children = $(this).find(' > ul > li');
        children.hide();
        $(this).find(" > span").attr('title', 'Click to open').find(' > i').addClass('fa-plus-square-o').removeClass('fa-minus-square-o');
    });
...
$("body").on("click", ".tree li.parent_li_lecture > span", function (e) {
    var $this = $(this);
    var $parent_li = $this.parent('li.parent_li_lecture');
    var children = $parent_li.find(' > ul > li');
    if (children.length > 0) {
        if (children.is(":visible")) {
            children.hide('fast');
